...
func main() {
	...
	nauir := types.NewNsqAdminUpdateImageRequest(name, namespace, image)

	// Customize wait timeout
	//wt := 180 * time.Second
	//nauir.SetWaitTimeout(wt)

	err = sdkv1alpha1.UpdateNsqAdminImage(kubeClient, nsqClient, nauir)
	if err != nil {
		klog.Fatalf("Update nsqadmin %s/%s to image %s error: %v", nauir.Namespace, nauir.Name, nauir.Image, err)
	}

	klog.Infof("Update nsqadmin %s/%s to image %s success", nauir.Namespace, nauir.Name, nauir.Image)
}
