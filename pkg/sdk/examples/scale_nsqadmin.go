...
func main() {
	...
	err = sdkv1alpha1.ScaleNsqAdmin(nsqClient, nasr)
	if err != nil {
		klog.Fatalf("Scale nsqadmin %s/%s to replicas %d error: %v", nasr.Namespace, nasr.Name, nasr.Replicas, err)
	}

	klog.Infof("Scale nsqadmin %s/%s to replicas %d success", nasr.Namespace, nasr.Name, nasr.Replicas)
}
